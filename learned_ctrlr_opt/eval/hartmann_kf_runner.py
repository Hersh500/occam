...
def hartmann_kf_runner(experiment_cfg, random_seed, adapt=True):
    ...
    kf_guesses = np.zeros((len(test_params_array), num_trials, len(kf_cfg.gains_to_optimize)))
    ...
    for p, params in enumerate(test_params_array):
        hartmann_env = Hartmann(params, kf_cfg.gains_to_optimize)
