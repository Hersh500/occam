...
def hartmann_reptile_runner(experiment_cfg, random_seed):
    ...
    reptile_guesses = np.zeros((len(test_params_array), num_trials, len(reptile_cfg.gains_to_optimize)))
    expected_scaled_metrics = np.zeros((len(test_params_array), num_trials, 1))
    for p, params in enumerate(test_params_array):
        hartmann_env = Hartmann(params, reptile_cfg.gains_to_optimize)
        ...
        for trial in range(num_trials):
            ...
            best_gain, best_acq_cost, best_surr_cost = random_search_simple(experiment_cfg.num_search_samples, eval_fn_reptile, gain_dim,
                                                                            HartmannInputs.get_bounds()[reptile_cfg.gains_to_optimize])
            true_value = hartmann_env.evaluate_x(best_gain)
