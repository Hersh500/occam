...
def mob_kf_runner(experiment_cfg, random_seeds, adapt=True):
    ...
    # for these purposes keep params array as an array in this case
    ...
    expected_variances = np.zeros((len(random_seeds), len(cmds), len(params_array), num_trials, 1))
    ...
    expected_scaled_metrics = np.zeros((len(random_seeds), len(cmds), len(params_array), num_trials, len(kf_cfg.metric_idxs)))
    ...
    # gain_bounds_torch = torch.tensor(Go1BehaviorParams.get_bounds(), device=device)
    gaits_torch = torch.tensor(GAITS_NORM, dtype=torch.float32, device=device)
    def mob_sampler(q):
        samples = torch.rand(q, gain_dim, device=device)
        if 2 in kf_cfg.gains_to_optimize and 3 in kf_cfg.gains_to_optimize and 4 in kf_cfg.gains_to_optimize:
    ...
    for s_idx, seed in enumerate(random_seeds):
        ...
        for t_idx, cmd in enumerate(cmds):
            ...
            if experiment_cfg.no_meta:   # don't use meta-learned weights
                kf_network.initialize_priors()

            weights = [kf_network.last_layer_prior.clone().float().to(device) for w in range(n_envs)]
            sigmas = [torch.mm(kf_network.last_layer_prior_cov_sqrt,
                               torch.t(kf_network.last_layer_prior_cov_sqrt)).float().to(device) for w in
                      range(n_envs)]
            ...
            task_target_data = torch.zeros(n_envs, num_trials, len(kf_cfg.metric_idxs))
            ...
            for i in range(num_trials):
                ...
                for p_idx in range(params_array.shape[0]):
                    all_weights[s_idx,t_idx,p_idx,i] = weights[p_idx].detach().cpu().numpy()
                    # need to not reset this guy...
                    ...
                    def eval_fn(x, cost_weights, sigma_weight):
                        ...
                        if x.device != device:
                            x = x.to(device)
                        y_mean, y_var= last_layer_prediction_uncertainty_aware(x, kf_network, weights[p_idx], sigmas[p_idx])
                        cost_weights_batch = cost_weights.repeat(q).reshape((q, cost_weights.shape[-1]))
                        mean_losses = torch.sum(y_mean * cost_weights_batch, dim=-1)
                        variances = torch.zeros(x.size(0), device=device)
                        for j in range(x.size(0)):
                            inter = torch.mm(y_var[j], cost_weights.unsqueeze(-1))
                            variances[j] = torch.mm(torch.t(cost_weights.unsqueeze(-1)), inter)
                        losses = mean_losses + sigma_weight * variances
                        return losses, y_mean, variances

                    traj_lim = obtained_trajs[p_idx, -kf_cfg.history_length:]
                    ...
                    input_test = torch.cat([best_gain, torch.from_numpy(cmd_scaled), torch.from_numpy(traj_flat)],
                                           dim=-1).unsqueeze(0)
                    best_y_mean, best_y_sigma = last_layer_prediction_uncertainty_aware(input_test, kf_network, weights[p_idx],
                                                                                        sigmas[p_idx])
                    expected_scaled_metrics[s_idx, t_idx, p_idx, i] = best_y_mean.cpu().detach().numpy()
                    expected_variances[
                        s_idx, t_idx, p_idx, i] = cost_weights.T @ best_y_sigma.cpu().detach().numpy() @ cost_weights
                    task_input_data[p_idx, i] = input_test
                    full_traj[s_idx, t_idx, p_idx, i*traj_len:(i+1)*traj_len] = obtained_trajs[p_idx]
                search_time_end = time.time()
                # print(f"Search time was {search_time_end - search_time_start}")
                ...
                # print(f"Scaled perf metrics were {perf_metrics_scaled}")
                ...
                # do weight adaptation step
                for p_idx in range(params_array.shape[0]):
                    if success_mask[p_idx]:
                        phi = kf_network(task_input_data[p_idx, i].unsqueeze(0).float().to(device)).squeeze().detach()
                        if adapt:
                            for r_step in range(experiment_cfg.num_replay_steps + 1):
                                with torch.no_grad():
                                    weights[p_idx], sigmas[p_idx], K = kalman_step(weights[p_idx],
                                                                                   sigmas[p_idx],
                                                                                   task_target_data[p_idx,i].float().to(device),
                                                                                   phi,
                                                                                   Q,
                                                                                   R)
    ...
    robot.env.close()
    # doing this to match with the format of the other runners
    results = []
    for s_idx, seed in enumerate(random_seeds):
        result = ({"expected_costs": expected_costs[s_idx],
                   "expected_variances": expected_variances[s_idx],
...
def mob_kf_runner_idx(experiment_cfg, random_seed, cmd_idx, param_idx, adapt=True):
    kf_checkpoint_dir = experiment_cfg.kf_ckpt_dir
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
    kf_cfg = OmegaConf.load(os.path.join(kf_checkpoint_dir, "config.yaml"))
    gain_dim = len(kf_cfg.gains_to_optimize)
    ...
    n_envs = 1
    print(f"Creating {n_envs} parallel envs")
    np.random.seed(random_seed + cmd_idx)
    torch.manual_seed(random_seed + cmd_idx)
    robot = MoBLocomotion_ResetFree(gains_to_optimize=kf_cfg.gains_to_optimize,
                                    params=params_array,
                                    num_envs=n_envs,
                                    render=True,
