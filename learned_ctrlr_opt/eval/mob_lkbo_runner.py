...
from learned_ctrlr_opt.meta_learning.lkbo import *
...
def mob_lkbo_runner(experiment_cfg, random_seeds, adapt=True):
    ...
    for s_idx, seed in enumerate(random_seeds):
        ...
        for t_idx, cmd in enumerate(cmds):
            ...
            task_target_data = torch.zeros(n_envs, num_trials, 1)
            ...
            for i in range(num_trials):
                ...
                for p_idx in range(params_array.shape[0]):
                    ...
                    if i > experiment_cfg.lkbo_num_explore_iters:
                        gp_model, mll = train_dk_gp(task_input_data[p_idx, :i],
                                                    task_target_data[p_idx, :i].squeeze(),
                                                    kf_network,
                    ...
                    if i > experiment_cfg.lkbo_num_explore_iters:
                        var = gp_model(input_test.to(device)).variance.cpu().detach().numpy()
                    else:
                        var = 0
                    expected_variances[
                        s_idx, t_idx, p_idx, i] = var
                ...
                task_target_data[:,i] = torch.from_numpy(true_y_scaled).unsqueeze(-1)
