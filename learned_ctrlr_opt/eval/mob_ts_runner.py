...
def mob_ts_runner(experiment_cfg, random_seeds):
    ...
    ref_track_scaler = MinMaxScaler(clip=True).fit(ref_tracks_enc.reshape(-1, experiment_cfg.ts_lookahead_dim))
    dset_f.close()
    ...
    raw_metrics = np.zeros((len(random_seeds), len(cmds), len(params_array), num_trials, len(ts_cfg.metric_idxs)))
    full_traj = np.zeros((len(random_seeds), len(cmds), len(params_array), traj_len * num_trials, ts_cfg.traj_dim))
    tried_gains = np.zeros((len(random_seeds), len(cmds), len(params_array), num_trials, gain_dim))
    expected_scaled_metrics = np.zeros((len(random_seeds), len(cmds), len(params_array), num_trials, len(ts_cfg.metric_idxs)))
    actual_scaled_metrics = np.zeros((len(random_seeds), len(cmds), len(params_array), num_trials, len(ts_cfg.metric_idxs)))
    latents = np.zeros((len(random_seeds), len(cmds), len(params_array), num_trials, ts_cfg.latent_dim))
    failed_counts = np.zeros(len(random_seeds))

    n_envs = params_array.shape[0]
    print(f"Creating {n_envs} parallel envs")
    robot = MoBLocomotion_ResetFree(gains_to_optimize=ts_cfg.gains_to_optimize,
                                    params=params_array,
                                    num_envs=n_envs,
                                    render=False,
    ...
    def mob_sampler(q):
        ...
        if 2 in ts_cfg.gains_to_optimize and 3 in ts_cfg.gains_to_optimize and 4 in ts_cfg.gains_to_optimize:
    ...
    for s_idx, seed in enumerate(random_seeds):
        ...
        for t_idx, cmd in enumerate(cmds):
            ...
            task_input_data = torch.zeros(n_envs, num_trials, gain_dim + experiment_cfg.ts_lookahead_dim + history_in_size)
            task_target_data = torch.zeros(n_envs, num_trials, len(ts_cfg.metric_idxs))
            ...
            for i in range(num_trials):
                ...
                for p_idx in range(params_array.shape[0]):
                    ...
                    traj_lim = obtained_trajs[p_idx, -ts_cfg.history_length:]
                    ...
                    latents[s_idx, t_idx, p_idx, i] = latent.detach().cpu().numpy()
                ...
                perf_metrics[...,ts_cfg.metric_idxs_to_invert] = 1/(1+perf_metrics[...,ts_cfg.metric_idxs_to_invert])
                perf_metrics_scaled = metric_scaler.transform(perf_metrics[...,ts_cfg.metric_idxs])
                ...
                raw_metrics[s_idx,t_idx,success_mask,i] = perf_metrics[success_mask][:,ts_cfg.metric_idxs]
