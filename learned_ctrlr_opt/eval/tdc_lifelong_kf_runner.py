...
def tdc_lifelong_kf_runner(experiment_cfg, random_seed, adapt=True):
    ...
    try:
        for s_idx, seed in enumerate(seeds):
            ...
            torch.manual_seed(random_seed + s_idx)

            weights = kf_network.last_layer_prior
            sigma = torch.mm(kf_network.last_layer_prior_cov_sqrt,
                             torch.t(kf_network.last_layer_prior_cov_sqrt)).float().to(device)
            ...
            task_input_data = torch.zeros(len(params_array)*num_trials, gain_dim + lookahead_dim + history_in_size)
            task_target_data = torch.zeros(len(params_array)*num_trials, len(kf_cfg.metric_idxs))
            ...
            for p_idx, params in enumerate(params_array):
                ...
                for i in range(num_trials):
                    ...
                    all_weights[s_idx, which_idx] = weights.cpu().detach().numpy()
                    ...
                    track_segments[s_idx, which_idx] = track_ahead
                    ...
                    if i > 0:
                        perturbed_optima = [task_input_data[:which_idx, :gain_dim]]
                        for num_to_add in range(5):
                            noisy_task_input_data = torch.clip(task_input_data[:which_idx, :gain_dim] + torch.randn(
                                task_input_data[:which_idx, :gain_dim].shape) * 3e-2, 0, 1)
                            perturbed_optima.append(noisy_task_input_data)
                        both_task_input_data = torch.cat(perturbed_optima, dim=0)
                    else:
                        both_task_input_data = task_input_data[:which_idx, :gain_dim]
                    ...
                    tried_gains[s_idx, which_idx, :] = best_gain.detach().cpu().numpy()
                    ...
                    raw_metrics[s_idx, which_idx] = perf_metrics[kf_cfg.metric_idxs]
                    ...
                    expected_costs[s_idx, which_idx] = best_y
                    actual_costs[s_idx, which_idx] = true_y_scaled
                    actual_scaled_metrics[s_idx, which_idx] = metric_scaler.transform(
                        perf_metrics[kf_cfg.metric_idxs].reshape(1, -1)).squeeze()
                    ...
                    best_y_mean, best_y_sigma = last_layer_prediction_uncertainty_aware(input_test, kf_network, weights,
                                                                                        sigma)
                    expected_scaled_metrics[s_idx, which_idx] = best_y_mean.cpu().detach().numpy()
                    expected_variances[
                        s_idx, which_idx] = cost_weights.T @ best_y_sigma.cpu().detach().numpy() @ cost_weights
                    ...
                    task_input_data[which_idx] = inp
                    task_target_data[which_idx] = target
                    if adapt:
                        with torch.no_grad():
