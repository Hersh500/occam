...
def tdc_rf_nom_gain_runner(experiment_cfg, random_seed):
    ...
    for s_idx, seed in enumerate(seeds):
        for p_idx, params in enumerate(params_array):
            ...
            for i in range(num_trials):
                ...
                raw_metrics_ig[s_idx, p_idx, i] = perf_metrics[kf_cfg.metric_idxs]
                ...
                actual_costs_ig[s_idx, p_idx, i] = true_y_scaled
                actual_scaled_metrics_ig[s_idx, p_idx, i] = metric_scaler.transform(
                    perf_metrics[kf_cfg.metric_idxs].reshape(1, -1)).squeeze()
...
def tdc_rf_nom_gain_runner_idx(experiment_cfg, random_seed, track_idx, param_idx):
    kf_checkpoint_dir = experiment_cfg.kf_ckpt_dir
    kf_cfg = OmegaConf.load(os.path.join(kf_checkpoint_dir, "config.yaml"))
    # load scalers
    scalers = get_scalers(kf_cfg.path_to_dataset, kf_cfg.history_length, kf_cfg.metric_idxs,
    ...
    metrics, traj, env = robot.evaluate_x(initial_gain,
                                          render=True,
                                          vid_folder="videos/",
                                          vid_prefix=f"tdc_nom_gain_{random_seed}_{track_idx}_{param_idx}")
    success = eval_success(traj)
    if not success:
        print("Could not find an initial gain that worked!")
        return
